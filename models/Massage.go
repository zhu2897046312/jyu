...
func udpSendProc() {
	...
	for {
		...
		case data := <-udpsendChannel:
			log.Println("this is udpSendProc") // 打印日志，表示执行 UDP 发送处理函数
			// 向 UDP 地址发送数据
...
func udpRecvProc() {
	...
	for {
		...
		log.Println("this is udpRecvProc") // 打印日志，表示执行 UDP 接收处理函数
...
func sendMsg(account int64, msg []byte) {
	rwLocker.RLock()
